...
def read_dataframe(filename: str):
    """
    It Loads a parquet file and add a numeric trip-duration column.
    Works for both Green (lpep_*) and Yellow (tpep_*) taxi datasets.
    """
    df = pd.read_parquet(filename)

    # it detects which prefix the file uses
    if "tpep_pickup_datetime" in df.columns:
        pickup_col  = "tpep_pickup_datetime"
        dropoff_col = "tpep_dropoff_datetime"
    else:  # fall backs to the green-cab schema
        pickup_col  = "lpep_pickup_datetime"
        dropoff_col = "lpep_dropoff_datetime"

    # it computes trip duration in minutes
    df["duration"] = (df[dropoff_col] - df[pickup_col]).dt.total_seconds() / 60

    # it keep only reasonable trips
...
@click.option(
    ...
    help="Dataset prefix that matches the parquet filenames"
)
